...
class myCalendar {
    ...
    public function initialize($ctx = 'web',$properties = array()) {
        if (empty($this->initialized[$ctx])) {
            ...
            if ($this->modx->getOption('loadjQueryUI', $this->config, true)) $this->modx->regClientCSS($this->config['cssUrl'].'jquery-ui.min.css');
            ...
            if ($this->modx->getOption('loadjQuery', $this->config, true)) $this->modx->regClientScript($this->config['jsUrl'].'lib/jquery.min.js');
            ...
            if ($this->modx->getOption('loadjQueryUI', $this->config, true)) $this->modx->regClientScript($this->config['jsUrl'].'lib/jquery-ui.min.js');
    ...
    public function removeEvent ($id = 0) {
        ...
        if ($this->config['readOnly']) return $this->error($this->modx->lexicon('mc.read_only'));
